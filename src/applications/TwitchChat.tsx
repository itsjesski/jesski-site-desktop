...
import { twitchAPI, type TwitchFollower, type TwitchSubscriber } from '../services/twitchAPI'
  ...
  useEffect(() => {
    const fetchActivityData = async () => {
      try {
        ...
        // Fetch followers and subscribers (using simulated data for now)
        ...
        const combinedActivity = [
          ...
          ...subscribersData.map((subscriber, index) => ({
            ...
            timestamp: new Date(Date.now() - (index + 1) * 480000).toISOString(), // Spread subs over time (8min intervals)
            data: subscriber
          }))
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 min-w-0">
        {chatError ? (
          ...
          <iframe
            ...
            title="Twitch Chat"
            onError={(e) => {
              console.error('Chat embed error:', e)
              setChatError(true)
            }}
            onLoad={() => {
              console.log('Chat embed loaded successfully')
            }}
          />
        )}
      </div>
      ...
      >
        {/* Header */}
        <div className="mb-4">
          <h2 className="text-lg font-bold text-gray-900 mb-1">Activity Feed</h2>
          <p className="text-xs text-gray-500">Fake updates from Jesski's stream</p>
        </div>
            ...
            <div className="space-y-3">
              ...
              {activityItems.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-400">Activity will appear here when you go live</p>
                </div>
              )}
            </div>
          </div>
